...
def confirm_item(request):
    if request.method == 'POST':
        ...
        for item_id in list_of_input_ids:
            ...
            if 'reject_items' in request.POST:
                item_id.delete()
            elif 'accept_items' in request.POST:
...
def delete_from_cart(request, id=None):
    ...
    if item.item_set.all():
        for i in item.item_set.all():
            if i in shop_cart.items.all():
                i_ex = get_object_or_404(ItemExtra, itemex=i, shopcart=shop_cart)
                i_ex.delete()
                shop_cart.items.remove(i)
    itemex.delete()
